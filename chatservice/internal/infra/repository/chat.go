...
import (
	...
	"github.com/dbelarmino/fclx/chat-service/internal/infra/db"
)
...
func (r *ChatRepositoryMySQL) CreateChat(ctx context.Context, chat *entity.Chat) error {
	err := r.Queries.CreateChat(ctx, db.CreateChatParams{
		ID:               chat.ID,
		UserID:           chat.UserID,
		InitialMessageID: chat.InitalSystemMessage.Content,
		Status:           chat.Status,
	...
	err = r.Queries.AddMessage(ctx, db.AddMessageParams{
		ID:        chat.InitalSystemMessage.ID,
		ChatID:    chat.ID,
		Content:   chat.InitalSystemMessage.Content,
		Role:      chat.InitalSystemMessage.Role,
		Tokens:    int32(chat.InitalSystemMessage.Tokens),
		CreatedAt: chat.InitalSystemMessage.CreatedAt,
	})
